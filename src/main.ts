// @ts-nocheck — staged port. The existing 4400-line app body still uses
// the legacy global references (`fabric`, `pdfjsLib`, `PDFLib`) that the
// CDN scripts injected as window globals. The imports below restore those
// globals from the npm packages so the body keeps working unchanged. Type
// safety + module split is the next pass; the goal here is a working
// Vite-bundled baseline that mirrors the previous CDN behavior.

import "./styles.css";
import { fabric } from "fabric";
import * as pdfjsLib from "pdfjs-dist";
// pdf.js worker import — Vite bundles it as a real worker file. The
// `?worker&url` suffix tells Vite to emit it as an asset and return its
// resolved URL, which we then point the global worker config at.
import pdfjsWorker from "pdfjs-dist/build/pdf.worker.min.js?url";
import * as PDFLib from "pdf-lib";

// Restore the globals the existing body relies on. Once the body is
// fully refactored to import these directly, these assignments go away.
(window as any).fabric = fabric;
(window as any).pdfjsLib = pdfjsLib;
(window as any).PDFLib = PDFLib;
pdfjsLib.GlobalWorkerOptions.workerSrc = pdfjsWorker;
import {
    CLIPBOARD_OBJECT_PROPS, KEYBOARD_PASTE_OFFSET,
    DEFAULT_MAX_STROKE_SIZE, DEFAULT_MAX_TEXT_SIZE,
    PAGE_SCALE_ADJUST, ARROW_MODE_OPTIONS, HIGHLIGHT_MODE_OPTIONS,
    PREF_KEY_ARROW_MODE, PREF_KEY_HIGHLIGHT_MODE, PREF_KEY_RECENT_COLORS,
    PREF_KEY_SHORTCUTS, PREF_KEY_EDITOR_SETTINGS, PREF_KEY_TOOLBAR_STATE,
    TOOL_NAMES, SHAPE_TOOLS,
    SHORTCUT_SCHEMA_VERSION, SHORTCUT_MODIFIER_CODES,
    SHORTCUT_TOOL_FIELDS, SHORTCUT_ACTION_FIELDS, SHORTCUT_REFERENCE_ACTION_IDS,
    SHORTCUT_TOOL_FIELD_MAP, SHORTCUT_ACTION_FIELD_MAP,
    EDITOR_SETTINGS_SCHEMA_VERSION, SETTINGS_EXPORT_SCHEMA_VERSION,
    TOOLBAR_STATE_SCHEMA_VERSION, FONT_FAMILY_OPTIONS, EDITOR_SETTINGS_LIMITS,
    MAX_HISTORY, HIGHLIGHT_LAYER_ALPHA, PDF_REFRESH_MARGIN_PX,
    SAVE_OVERLAY_SCALE, ZOOM_STEP, ZOOM_MIN, ZOOM_MAX,
} from "./constants";
